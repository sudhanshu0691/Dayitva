  ...
  if (!hydrated) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="border-2 border-accent border-t-transparent w-6 h-6 animate-spin rounded-full" />
      </div>
    );
  }

  return (
    <div className="w-full max-w-6xl mx-auto px-6 py-10 space-y-8 relative perspective">
      ...
      {/* KYC Banner */}
      <div className="border border-border bg-card rounded-xl p-5 card-depth relative z-10">
        {!isKycApproved ? (
          <div className="flex items-start gap-3">
            <AlertCircle className="w-5 h-5 text-warning mt-0.5 shrink-0" />
            <div className="flex-1">
              <h3 className="text-body-sm font-semibold text-warning">KYC verification required</h3>
              ...
              <button onClick={() => router.push("/vendor/profile")}
                className="mt-3 px-4 py-2.5 bg-warning text-warning-foreground text-label-sm rounded-lg hover:opacity-90 transition-opacity font-semibold min-h-[44px]">
                Complete KYC now
              </button>
            </div>
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-warning/30 bg-warning/10 text-warning text-label-sm font-semibold rounded-full">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-body-sm font-semibold text-success">KYC verified</h3>
              <p className="text-label-sm text-muted-foreground mt-1">
                Your KYC is approved. You can now place bids on open tenders.
              </p>
            </div>
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-success/30 bg-success/10 text-success text-label-sm font-semibold rounded-full">
      ...
      {/* Live Tenders */}
      <div>
        <h2 className="text-headline-sm text-foreground font-semibold mb-4">Live tenders</h2>
        
        {loading ? (
          <div className="space-y-3">
            {[1,2,3].map(i => (
              <div key={i} className="animate-pulse border border-border bg-card/30 rounded-lg p-5">
          ...
          <div className="border border-dashed border-border/40 bg-card/20 p-10 text-center rounded-lg">
          ...
          <div className="space-y-3">
            {tenders.map((tender) => (
              <div key={tender.id} onClick={() => router.push(`/tenders/${tender.id}`)}
                className="card-3d-tilt border border-border bg-card rounded-lg p-5 cursor-pointer">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="text-body-sm font-semibold text-foreground">{tender.title}</h3>
                      {tender.isNew && (
                        <span className="px-1.5 py-0.5 bg-accent/10 text-accent text-[10px] font-semibold rounded-full">New</span>
                      )}
                    </div>
                    <p className="text-label-sm text-muted-foreground mt-1">
                ...
                <div className="flex items-center gap-4 mt-3 text-label-sm text-muted-foreground">
                  ...
                  {tender.txHash && (
                    <a href={`https://sepolia.etherscan.io/tx/${tender.txHash}`}
                      ...
                      className="flex items-center gap-1 text-accent hover:text-accent/80 font-semibold"
