 ...
 */
function formatTender(tender: any): ITender {
  const bids = (tender.bids || []).map((b: any) => formatBid(b));
  const auditTimeline = (tender.auditLogs || []).map((a: any) => formatAuditLog(a));
  const ipfsFiles = (tender.tenderFiles || []).map((f: any) => ({
    name: f.name,
    size: f.size,
    hash: f.hash,
  ...
  return {
    ...
    status: tender.status as ITender["status"],
...
function formatBid(bid: any): IBid {
  return {
    id: bid.id,
...
function formatAuditLog(log: any): IAuditStep {
  return {
    id: log.id,
    title: log.title,
    ...
    iconType: log.iconType as IAuditStep["iconType"],
  };
}
 ...
 */
export async function listTenders(query: TenderQueryInput) {
 ...
 */
export async function updateTender(tenderId: string, input: UpdateTenderInput, officerId: string) {
  ...
  if (tender.officerId !== officerId) {
    throw new AppError("You can only update your own tenders", 403);
  }

  if (tender.status !== "Draft" && tender.status !== "Open") {
    throw new AppError("Cannot update tender after bidding has closed", 400);
  }
  ...
  if (input.msmeQuota !== undefined) updateData.msmeQuota = input.msmeQuota;
  if (input.criteria !== undefined) updateData.criteria = JSON.stringify(input.criteria);
  if (input.ipfsHash !== undefined) updateData.ipfsHash = input.ipfsHash;
 ...
 */
export async function updateTenderStatus(tenderId: string, status: string, officerId: string) {
  const tender = await prisma.tender.findUnique({ where: { id: tenderId } });
  ...
  if (tender.officerId !== officerId) {
    throw new AppError("You can only modify your own tenders", 403);
  }
 ...
 */
export async function deleteTender(tenderId: string, officerId: string) {
  const tender = await prisma.tender.findUnique({
    where: { id: tenderId },
  ...
  if (tender.officerId !== officerId) {
    throw new AppError("You can only delete your own tenders", 403);
  }

  if (tender._count.bids > 0) {
    throw new AppError("Cannot delete tender with existing bids", 400);
  }
