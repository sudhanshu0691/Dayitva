 ...
 */
function computePriceScore(price: number, budget: number): number {
 ...
 *         + (proposalQuality × 5%)
 */
export function calculateBidScore(params: IBidScoreParams): IBidScoreResult {
 ...
 */
export function determineWinner(bids: Array<{
  id: string;
  vendorId: string;
  vendorName: string;
